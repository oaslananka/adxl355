...
|---|---|---|---|---|---|---|---|---|---|---|
| C | Yes | Yes | Bounded count/decode/read, caller storage | Yes, bounded measured response | Internal-clock DRDY/INT1/INT2 configuration; no GPIO loop | Example only | No | No | Yes, CMake install/export | Raspberry Pi 5 SPI response pass; DRDY GPIO pending |
...
| Python | Yes | Yes | Bounded count/decode/read, typed partial results | Yes, bounded measured response | Internal-clock DRDY/INT configuration plus bounded libgpiod reference | Yes, `spidev` | Yes, `smbus2` | No | Yes, sdist/wheel | Raspberry Pi 5 SPI and I2C feature passes; DRDY GPIO pending |
