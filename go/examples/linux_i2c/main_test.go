...
func TestParseOptionsUsesBoundedDefaults(t *testing.T) {
	...
	if config.bus != 1 || config.address != 0x1D || config.busHz != 400_000 || config.samples != 8 || config.timeout != 10*time.Second {
...
func TestParseOptionsRejectsUnsafeBounds(t *testing.T) {
	...
	if strconv.IntSize == 64 {
		cases = append(cases, []string{"-bus-hz", strconv.FormatUint(uint64(math.MaxUint32)+1, 10)})
	}
