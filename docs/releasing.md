...
3. match every maintained package manifest, lockfile version, shared vector
   version, and generated C version header;
...
check, builds without publishing, generates SHA-256 checksums, and uploads an
inspectable artifact. A final job downloads all package artifacts and creates an
...
a temporary fixture, intentionally changes `node/package.json` from `0.1.0` to
