...
- **Breaking (C/C++)**: Transport read/write callbacks now return the exact transferred byte count on success and a negative value on failure; all drivers reject zero, truncated, and overlong payloads and normalize backend failures to stable bus errors
